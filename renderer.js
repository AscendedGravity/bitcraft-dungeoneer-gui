...
function sanitizeDescription(raw, hasStructuredContrib) {
  ...
  const filteredLines = lines
    .filter(Boolean)
    .filter(
      (l) => !/^[\s\-]*current[_\s\-]*(?:players|layers)\b[:\s\-]*/i.test(l)
    );
  ...
  if (hasStructuredContrib) {
    ...
    for (let i = 0; i < filteredLines.length; i++) {
      ...
      if (
        /^player\s+contribution/i.test(l) ||
        /^boss contribution\s*:?/i.test(l) ||
        contributionLineRe.test(l) ||
        totalLineRe.test(l)
      ) {
        continue;
      }
      ...
      if (/^players?:\s*/i.test(l)) {
        ...
        while (j < filteredLines.length) {
          ...
          if (
            /^player\s+contribution/i.test(next) ||
            /^boss contribution\s*:?/i.test(next) ||
            contributionLineRe.test(next) ||
            totalLineRe.test(next) ||
            /^players?:\s*/i.test(next)
          )
            break;
          // skip this name line
          j++;
...
async function init() {
  ...
  window.__dungeoneer_snapshot_open_states =
    window.__dungeoneer_snapshot_open_states || {};
  ...
  async function loadList() {
    ...
    try {
        ...
        window.__dungeoneer_countdown_interval = setInterval(() => {
          try {
            ...
            try {
              ...
              const detailTimer = document.querySelector(
                "#detail-title .elapsed-timer"
              );
      ...
      for (const id of ids) {
        ...
        const rawTitle =
          data[id] && data[id].title ? String(data[id].title) : "";
        ...
        let kindText = "";
        if (data[id] && data[id].state && data[id].state.kind)
          kindText = String(data[id].state.kind);
        // create structured content: name/state on left, countdown on right (if ts present)
        const left = document.createElement("span");
        left.className = "dungeon-label";
        left.textContent = kindText
          ? `${clean(nameText)} — ${clean(kindText)}${playerCountSuffix(
              data[id],
              kindText
            )}`
          : `${clean(nameText)}${playerCountSuffix(data[id], kindText)}`;
        ...
        if (!kindText) {
          (async () => {
            try {
              ...
              const sk =
                dd && dd.state && dd.state.kind ? String(dd.state.kind) : "";
              if (sk)
                left.textContent = `${clean(nameText)} — ${clean(
                  sk
              ...
              try {
                ...
                if (tsFromDetail) {
                  ...
                  if (rightNode) {
                    ...
                    rightNode.textContent = formatCountdown(
                      tsFromDetail - Math.floor(Date.now() / 1000)
                    );
                  }
                }
              } catch (innerErr) {
        ...
        if (ts) {
          ...
          // initial value
          right.textContent = formatCountdown(
  ...
  function getNormalizedKind(entry) {
    ...
    try {
      if (entry.state && entry.state.kind)
        return String(entry.state.kind).toLowerCase();
      if (entry.title) {
  ...
  function updateElapsedStateForBtn(btn, entry, id) {
    window.__dungeoneer_elapsed = window.__dungeoneer_elapsed || {};
    const prev = window.__dungeoneer_elapsed[id] || {
      status: "hidden",
      startAt: 0,
      baseElapsed: 0,
    };
    ...
    const persistLastElapsed = (p) => {
      try {
        window.__dungeoneer_last_elapsed =
          window.__dungeoneer_last_elapsed || {};
        let last = 0;
        if (p) {
    ...
    // helper to reset
    const resetState = () => {
      window.__dungeoneer_elapsed[id] = {
        status: "hidden",
        startAt: 0,
        baseElapsed: 0,
        prevPlayers: players,
    ...
    if (isEmpty) {
      ...
      if (prev && prev.kind === "cleared" && kind === "closed") {
        ...
        resetState();
        const el = btn.querySelector(".elapsed-timer");
        if (el) {
      ...
      if (prev && prev.status === "running") {
        ...
        window.__dungeoneer_elapsed[id] = Object.assign({}, prev, {
          prevPlayers: players,
        });
      }
      return;
    ...
    if (kind === "cleared" && !(prev && prev.kind === "cleared")) {
      ...
      if (el) {
        el.style.display = "";
        el.textContent = formatElapsedMMSS(base);
      }
      return;
    }
    ...
    if (
      (prevWasEmpty || prev.status === "hidden") &&
      !isEmpty &&
      kind !== "closed" &&
      kind !== "cleared"
    ) {
      ...
      if (el) {
        el.style.display = "";
        el.textContent = formatElapsedMMSS(0);
      }
      return;
    }
  ...
  function applyStateClass(btn, dataEntry, id) {
    ...
    const titleText = String(
      (state && state.title) || (dataEntry && dataEntry.title) || ""
    ).toLowerCase();
    // prioritize boss detection by inspecting title text (covers cases like 'boss fight!')
    if (
      /\bboss\b/.test(titleText) ||
      (state && String(state.kind).toLowerCase() === "boss")
    ) {
    ...
    if (state && state.kind) {
      ...
      if (kind === "open") {
        if (!players || players.length === 0)
          btn.classList.add("state-open-empty");
  ...
  function playerCountSuffix(entry, kindTextOverride) {
    ...
    if (entry.state && entry.state.kind) {
      try {
        if (String(entry.state.kind).toLowerCase() === "closed")
          isClosed = true;
      } catch (e) {}
    }
    ...
    if (!isClosed && kindTextOverride) {
      try {
        if (String(kindTextOverride).toLowerCase().includes("closed"))
          isClosed = true;
      } catch (e) {}
    }
    ...
    if (!isClosed && entry.title) {
      try {
        if (String(entry.title).toLowerCase().includes("closed"))
          isClosed = true;
      } catch (e) {}
    }
    if (isClosed) return "";
  ...
  async function loadDetails(id) {
    ...
    try {
      ...
      const stateKind =
        d && d.state && d.state.kind ? String(d.state.kind) : stateText;
      const titleText = stateKind
        ? `${cleanText(nameText)} — ${cleanText(stateKind)}`
        : cleanText(nameText);
      ...
      if (titleTextSpan.textContent !== titleText)
      ...
      if (d.description) {
        ...
        const filteredLines = lines
          ...
          .filter(
            (l) =>
              !/^[\s\-]*current[_\s\-]*(?:players|layers)\b[:\s\-]*/i.test(l)
          );
      ...
      if (showDesc) {
        ...
        const cleanedDesc = desc
          ...
          .filter(Boolean)
          .join("\n");
        if (descEl.textContent !== cleanedDesc)
      ...
      // - No otherwise
      const titleLowerForBoss = String(
        (d.state && d.state.title) || d.title || ""
      ).toLowerCase();
      const kindLowerForBoss =
        d && d.state && d.state.kind ? String(d.state.kind).toLowerCase() : "";
      let atBossStatus = "No";
      if (/\bboss\b/.test(titleLowerForBoss) || kindLowerForBoss === "boss") {
        atBossStatus = "Yes";
      } else if (
        kindLowerForBoss === "cleared" ||
        kindLowerForBoss === "closed"
      ) {
      ...
      if (
        last.players !== playersJSON ||
        last.boss !== atBossStatus ||
        isSwitchingDungeon
      ) {
        playersEl.innerHTML = "";
        ...
        const bossLabel = document.createElement("strong");
        bossLabel.textContent = "At Boss:";
        bossDiv.appendChild(bossLabel);
        const bossVal = document.createElement("span");
        bossVal.style.marginLeft = "6px";
        bossVal.textContent = atBossStatus;
        bossDiv.appendChild(bossVal);
        playersEl.appendChild(bossDiv);
      }

      // contribution from latest run
      const contribHeaderHtml =
        '<h3>Contribution (Latest Run) <button id="copy-contributions" style="margin-left: 10px;">Copy</button></h3>';
      ...
      function parseContribution(item) {
        ...
        if (typeof item === "object") {
          ...
          const amount =
            item.amount || item.value || item.v || item.amt || item[0] || "";
          const percent =
            item.percent || item.pct || item.percent_of || item[1];
          ...
          return {
            player,
            amount: amount === undefined ? "" : String(amount).trim(),
            percent: percent === undefined ? "" : String(percent).trim(),
          };
        }
      ...
      function makeContribLi(parsed, isTotal) {
        ...
        const left = document.createElement("span");
        left.className = "contrib-player";
        left.textContent = parsed.player || "";
        const right = document.createElement("span");
        right.className = "contrib-value";
        ...
        if (isTotal) {
          ...
          strongLeft.appendChild(left);
          const strongRight = document.createElement("strong");
          strongRight.appendChild(right);
          li.appendChild(strongLeft);
          li.appendChild(strongRight);
        } else {
          li.appendChild(left);
      ...
      if (!contribHeader) {
        ...
        contribHeader.innerHTML =
          'Contribution (Latest Run) <button id="copy-contributions" class="copy-btn" style="margin-left: 10px;">Copy</button>';
      ...
      if (last.contrib !== contribListJson || isSwitchingDungeon) {
        ...
        const existingLists = contrib.querySelectorAll(
          ":scope > .contrib-entries, :scope > p"
        );
        existingLists.forEach((n) => n.remove());

        // insert contributions (or placeholder message) immediately after the header
        const snapContainer = contrib.querySelector(
          ":scope > .snapshots-container"
        );
        const insertBeforeNode =
          snapContainer || contribHeader.nextSibling || null;
        ...
        if (d.contributions && d.contributions.length) {
          ...
          for (const c of d.contributions) {
            ...
            const isTotal =
              parsed &&
              parsed.player &&
              parsed.player.toLowerCase().includes("total");
            cUl.appendChild(
              makeContribLi(
                parsed || { player: String(c), amount: "", percent: "" },
                isTotal
              )
            );
          }
          ...
          p.innerHTML =
            "<em>Contribution list will populate after boss is defeated... </em>";
          ...
          try {
            ...
            if (elapsedState && elapsedState.status) {
              ...
              if (elapsedState.status === "running") {
                elapsedSec =
                  nowSec -
                  (elapsedState.startAt || nowSec) +
                  (elapsedState.baseElapsed || 0);
      ...
      try {
        ...
        const rawSnaps = await window.api.fetchJSON(
          `/dungeondetails/${id}/contrib`
        );
        let snaps = rawSnaps;
          ...
          if (last.snaps !== snapsJson || isSwitchingDungeon) {
            ...
            for (let si = 0; si < snaps.length; si++) {
              ...
              if (snap && typeof snap === "object" && !Array.isArray(snap)) {
                if (Array.isArray(snap.contributions))
                  snapItems = snap.contributions;
              ...
              titleSpan.className = "snapshot-title";
              // if snapshot contains ts, show a humanized time
              if (snapTs) {
                const tsSec =
                  typeof snapTs === "number" && snapTs > 1e11
                    ? Math.floor(snapTs / 1000)
                    : Math.floor(Number(snapTs) || 0);
                const date = new Date(tsSec * 1000);
                titleSpan.textContent = `Snapshot ${
                  si + 1
                } — ${date.toLocaleString()}`;
              } else {
              ...
              const wasOpen = Boolean(
                window.__dungeoneer_snapshot_open_states[snapKey]
              );
              ...
              for (const item of snapItems) {
                ...
                const parsed = parseContribution(
                  raw && raw.player ? raw : typeof raw === "string" ? raw : raw
                );
                const isTotal =
                  parsed &&
                  parsed.player &&
                ...
                snapUl.appendChild(
                  makeContribLi(
                    parsed || { player: String(item), amount: "", percent: "" },
                    isTotal
                  )
                );
              }
              ...
              snapBox.appendChild(header);
              snapBox.appendChild(content);
