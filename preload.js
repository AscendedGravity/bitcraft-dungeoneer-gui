...
(() => {
  ...
  const injectCSP = (orig) => {
    ...
    const connectPart = orig ? `connect-src 'self' ${orig}` : "connect-src 'self'";
...
contextBridge.exposeInMainWorld('api', {
  fetchJSON: async (path) => {
    ...
    if (!base) {
      throw new Error('DUNGEONEER_API_BASE is not set. Set it in your environment before launching the app.');
    }
    const url = `${base}${path}`;
